...
public static void ordenarInsercao(Restaurante[] array, int tam){ //usar compareTo() para comparar Strings ######

  long tempoInicio = System.nanoTime();

  for(int i = 1; i < tam;i++){

    Restaurante restChave = array[i]; //######## preciso guardar o restaurante para trocar depois
    String chave = array[i].getCidade(); //######## a chave para ordenacao e cidade
    int j = i - 1; 

    while (j >= 0) {
      
    numComparacoes++; // Conta o teste que será feito abaixo
    if (array[j].getCidade().compareTo(chave) > 0) {
        array[j + 1] = array[j];
        numMovimentacoes++;
        j--;
    } else {
  ...
  double tempoTotal = (tempoFim - tempoInicio) / 1_000_000_000.0; //converte para segundos 
...
public static void criarLog(int numComparacoes, double tempoTotal){ //cria o log pedido com o num de comp e tempo
  ...
  try{
    ...
    FileWriter fw = new FileWriter(matricula + "_insercao.txt"); //fw cria um novo arquivo
...
public static void main(String[] args){
  ...
  ordenarInsercao(array, arrayTam); 
