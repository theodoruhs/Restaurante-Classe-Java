...
//================ NOVA FUNCAO INSERTION SORT ====================

public static void ordenarHeapsort(Restaurante[] array, int n){ //usar compareTo() para comparar Strings ######

  for(int tam = 1; tam < n; tam++){ //vou adicionando os elementos a heap... 

    construir(array,tam); //chamando construir para todo novo elemento que adiciono
  }//### ate aqui construi o heap

  for(int tam = n - 1; tam > 0; tam--){//comeco com a heap igual o tamanho do array e vou diminuindo

    swap(array,0,tam); //colocando o ultimo elemento na pos tam (tamanho da heap)
    numMovimentacoes += 3; 
    reconstruir(array,tam); 

  }//pronto, desfiz a heap e ordenei

}

public static void construir(Restaurante[] array, int tam){

  
 for(int i = tam; i > 0 && comparar(array,i,(i - 1)/2) > 0; i = (i - 1) /2){ //enquanto o filho for maior ...
    
    swap(array,i,(i - 1)/2); //vou trocar o filho com o pai
...
public static void reconstruir(Restaurante[] array,int tam){

  int i = 0;

  while(2 * i + 1 < tam){

    int filho = 2 * i + 1;//comeco com o filho esquerdo

    if(filho + 1 < tam && comparar(array,filho + 1,filho) > 0){filho += 1;}//se o direito for maior mudo para ele
   
    if(comparar(array,filho,i) > 0){
...
public static int comparar(Restaurante[] array, int i, int j) {
    Data d1 = array[i].dataAbertura;
    Data d2 = array[j].dataAbertura;

    numComparacoes++;//contabilizo uma comparacao

    // 1. Compara o ano
    if (d1.ano != d2.ano) return d1.ano - d2.ano;
    
    // 2. Se o ano for igual, compara o mes
    if (d1.mes != d2.mes) return d1.mes - d2.mes;
    
    // 3. Se o mes for igual, compara o dia
    if (d1.dia != d2.dia) return d1.dia - d2.dia;

    // 4. Se tudo for igual, usa o critério de desempate (Nome)
    return array[i].nome.compareTo(array[j].nome);
}
...
public static void criarLog(int numComparacoes, double tempoTotal){ //cria o log pedido com o num de comp e tempo
  
  try{

    FileWriter fw = new FileWriter(matricula + "_heapsort.txt"); //fw cria um novo arquivo
    PrintWriter pw = new PrintWriter(fw);     //printWriter vai escrever nesse arquivo

    pw.printf("%s\t%d\t%d\t%.6f",matricula,numComparacoes,numMovimentacoes,tempoTotal);
...
static String matricula = "1066294"; 
...
public static void main(String[] args){
  ...
  Restaurante[] array = new Restaurante[colecao.getTamanho()]; //######## esse vai ser o array que vou ordenar #############
  int arrayTam = 0;
  ...
  while(idBuscado != -1){//############ funcao alterada para criar um array
    ...
    for(; i < tam;i++){//vou achar o restaurante cujo id seja igual o id buscado
      ...
      if(id == idBuscado){break;} //vou dar break se achar o id, e ai o [i] vai corresponder ao pos do id
    }

    array[arrayTam++] = lista[i];//vou por o restaurante cujo id correspondeu no novo array ############

    // LEITURA SEGURA:
    if (sc.hasNextInt()) {
        idBuscado = sc.nextInt();
        if (sc.hasNextLine()) {
            sc.nextLine(); // Só lê a linha se ela existir
        }
    } else {
        idBuscado = -1; // Encerra o loop se não houver mais números
    }


  }//Ate aqui gerei um array dos restaurantes com ID pedido que sera ordenado
  ...
  ordenarHeapsort(array,arrayTam);

  long tempoFim = System.nanoTime();
  double tempoTotal = (tempoFim - tempoInicio) / 1_000_000_000.0;

  //======== agora printo o array ordenado!!!

  for(int i = 0; i < arrayTam; i++){

    //chamamos o metodo formatar() do restaurante atual e printamos a string resultante
    System.out.println(array[i].formatar());

  }
  criarLog(numComparacoes, tempoTotal);
